...
# Daten einlesen und Vorbereitung ----------------------------------------------
...
# räumliche rarefaction curves -------------------------------------------------

# Vorbereitung der Daten für die räumliche Analyse

# Koordinaten der samples in den Fragmenten
xy_frag01 <-  frag_low %>%
  select(loc_x, loc_y)
...
# Jetzt berechne ich die sSBR (räumliche rarefaction curves)
sSBR_frag01 <- SCR(comm = frag01_spec, xy_coords = xy_frag01)
sSBR_frag05 <- SCR(comm = frag05_spec, xy_coords = xy_frag05)
...
ggplot(sSBR_data, aes(distance, S, color = fragmentation)) +
  geom_point() +
  geom_smooth()
